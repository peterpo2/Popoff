...
export const Sidebar: React.FC = () => {
  return (
    <aside className="w-64 shrink-0 px-5 py-6 glass-panel border-r border-white/5 relative z-10">
      <div className="mb-10">
      ...
      <nav className="space-y-2">
        {navItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition hover:bg-white/5 hover:text-text ${
                isActive ? 'bg-white/10 text-text shadow-glow' : 'text-primary'
              }`
            }
          >
